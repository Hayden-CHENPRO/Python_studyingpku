# !/usr/bin/env python
# -*- Coding:utf-8 -*-
# Created by Hayden at 2019/12/15
# 数据截止至12月15号
...
def dataTag(plot):
    for each in plot:    # 显示柱形图的数据标签
        height = each.get_height()
        plt.text(each.get_x()+each.get_width()/2, height, height, ha='center', va='bottom')


# 播放量单项分析
# x_label_value = pd.Series(['0~10', '10~100', '100~1000', '1000~10000', '10000~40000'])
# y_value_10 = df_plays[(df_plays <= 10)].shape[0]
# y_value_100 = df_plays[(df_plays > 10) & (df_plays <= 100)].shape[0]
# y_value_1000 = df_plays[(df_plays > 100) & (df_plays <= 1000)].shape[0]
# y_value_10000 = df_plays[(df_plays > 1000) & (df_plays <= 10000)].shape[0]
# y_value_40000 = df_plays[(df_plays > 10000) & (df_plays <= 40000)].shape[0]
# y_label_value = pd.Series([y_value_10, y_value_100, y_value_1000, y_value_10000, y_value_40000])
#
# plt.figure(figsize=(8,5))    # 设置画布尺寸
# plt.tick_params(labelsize=12)
# plt.title('播放量标准下的番剧分布', size=20, family='STSong')    # 标题，字号大小为20，中文字体类型
# plt.xlabel('播放量/万', size=14, family='STSong')
# plt.ylabel('番剧数/部', size=14, family='STSong')
# plt.ylim((0, 1000))    # 设置纵坐标范围
# plays_plot = plt.bar(x_label_value, y_label_value, width=0.5, color='k', alpha=0.7)
# dataTag(plays_plot)
# plt.show()
...
# y_value_2 = df_avg_grade[(df_avg_grade <= 2)].shape[0]
# y_value_5 = df_avg_grade[(df_avg_grade > 2) & (df_avg_grade <= 5)].shape[0]
# y_value_7 = df_avg_grade[(df_avg_grade > 5) & (df_avg_grade <= 7)].shape[0]
# y_value_8 = df_avg_grade[(df_avg_grade > 7) & (df_avg_grade <= 8)].shape[0]
# y_value_9 = df_avg_grade[(df_avg_grade > 8) & (df_avg_grade <= 9)].shape[0]
# y_value_95 = df_avg_grade[(df_avg_grade > 9) & (df_avg_grade <= 9.5)].shape[0]
# y_value_10 = df_avg_grade[(df_avg_grade > 9.5) & (df_avg_grade <= 10)].shape[0]
...
# plt.title('评分标准下的番剧分布', size=20, family='STSong')    # 标题，字号大小为20，中文字体类型
# plt.xlabel('总评分', size=14, family='STSong')
# plt.ylabel('番剧数/部', size=14, family='STSong')
# plt.ylim((0, 1000))    # 设置纵坐标范围
