...
def  numtify_with_unit(df, col_name):
    df2 = pd.Series([])
...
def numtify_series(df, col_name):
    df2 = pd.Series([])
...
def tag_statistics(df, col_name):
    tags_count = {}
...
def ch_judge_count(ch_list, strs, dicts):
    j = 0
    for ch in ch_list:
        if ch in strs:
...
def actor_statistics(df, col_name):
    ch_list = ['\\u002F', '、', '／', '+']
    actor_count = {}
        ...
        if each == '':
            ...
            try:
                ...
                for peop in actor_list:
                    ...
                    ch_judge_count(ch_list, actor, actor_count)
            except IndexError:
                continue
    return actor_count

def staff_statistics(df, col_name):
    ch_list = ['\\u002F', '、', '／', '+']
    director_count = {}
    author_count = {}
    df_inside = df[col_name].fillna('')
            ...
            try:
                ...
                for peop in staff_list:
                    ...
                    if work in ['导演', '监督', '总导演', '总监督']:
                        ch_judge_count(ch_list, stf, director_count)
                    elif work in ['原作', '企划', '企画']:
                        ch_judge_count(ch_list, stf, author_count)
                    else:
                        continue
            except  IndexError:
                continue
...
df_plays = numtify_with_unit(df, '播放量')
df_barCounts = numtify_with_unit(df, '弹幕量')
df_follows = numtify_with_unit(df, '追番数')
df_gradeCounts = df['评分人数'].fillna(0)
df_avg_grade = df['总评分'].fillna(0)
df_release_time = datify(df, '开播时间')
df_series = numtify_series(df, '剧集数')   # 统计数据不包含连载中的十月新番
df_tags = tag_statistics(df, '标签')    # 返回每个标签的统计个数
df_actor = actor_statistics(df, 'actor')    # 返回一个声优的统计个数字典
df_staff = staff_statistics(df, 'staff')    # 返回一个职员的统计个数字典
